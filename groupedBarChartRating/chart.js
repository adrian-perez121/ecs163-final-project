...
(function () {
  ...
  // Y tick values matching the screenshot
  const yTicks = [1.25, 2.5, 3.75, 5.0, 6.25, 7.5, 8.75, 10.0];
  ...
  g.append("g")
    ...
    .call(
      d3.axisLeft(yScale)
        .tickValues(yTicks)
        .tickFormat(d => d % 1 === 0 ? d.toFixed(0) : d.toFixed(2))
        .tickSizeOuter(0)
    );
    ...
    d3.select(this)
      ...
      .on("mousemove", function (event, d) {
        tooltip
          ...
          .html(`<strong>${d.cat}</strong>Era: ${d.era}<br/>Avg Rating: ${d.value.toFixed(2)}`)
